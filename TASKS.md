...
- [x] フロントエンド: フォルダパス表示 ✅ 実装済み
  ...
  - [x] フォルダクリックで権限一覧表示
  - [x] Google Driveへのリンク
- [x] フロントエンド: フォルダビュー ✅ 実装済み
  - [x] 表示モード切替（ファイル表示/フォルダ表示）
  - [x] フォルダ別ファイルグルーピング表示
  - [x] フォルダ展開/折りたたみ
...
- [x] OAuthスコープ変更 ✅ 実装済み
  - [x] `drive.readonly` → `drive` スコープに変更
  - [x] Google Cloud Console で同意画面更新
  - [x] 再認証フロー実装
...
- [x] バックエンド: フォルダ自体の権限変更API ✅ 実装済み
  - [x] `DELETE /api/scan/:scanId/folders/:folderId/folder-permissions/:permissionId` （フォルダ権限削除）
  - [x] `PUT /api/scan/:scanId/folders/:folderId/folder-permissions/:permissionId` （フォルダ権限更新）
- [x] バックエンド: 監査ログ ✅ 実装済み
...
- [x] フロントエンド: フォルダ権限変更 ✅ 実装済み
  - [x] フォルダ権限表示に「削除」ボタン追加
  - [x] フォルダ権限表示に「閲覧者に変更」ボタン追加
  - [x] フォルダ権限変更の確認ダイアログ
  - [x] フォルダ権限変更APIとの連携
  - [x] 操作結果トースト表示
- [x] フロントエンド: 一括操作 ✅ 実装済み（Milestone 7で完了）
  - [x] ファイル複数選択UI
  - [x] 一括アクション選択ダイアログ
  - [x] ドライラン（プレビュー）機能
  - [x] 実行結果サマリー表示
...
- [ ] 手動機能テスト（リリース前必須）✅ 2025-11-30 テスト実施
  ...
  - [ ] **管理者ダッシュボード** (`/admin`) ⏳ 未テスト
    - [ ] ページタイトル「管理者ダッシュボード」が表示される
    - [ ] ヘッダー・サイドメニューが表示される
    - [ ] 統計サマリーカード（総ユーザー数、スキャン実施済み、未実施ユーザー、総リスクファイル）
    - [ ] 組織全体のリスク状況（Critical/High/Medium/Low件数）
    - [ ] ユーザー別スキャン状況テーブル
      - [ ] ユーザー名・メール・ロールが表示される
      - [ ] 最終スキャン日時が表示される
      - [ ] ステータス（完了/実行中/失敗/未実施）が表示される
      - [ ] リスクファイル件数が表示される
      - [ ] 「詳細」ボタンでスキャン履歴が展開される
    - [ ] メンバー権限のユーザーはアクセス不可（警告表示）
  - [ ] **レポートページ** (`/reports`) ⏳ 未テスト
    - [ ] ページタイトル「ISMS/Pマーク対応レポート」が表示される
    - [ ] レポート選択カードが5種類表示される
      - [ ] スキャン実施履歴レポート（ISMS A.8.15対応）
      - [ ] リスクアセスメントレポート（ISMS A.5.9対応）
      - [ ] 是正対応履歴レポート（ISMS A.5.18対応）
      - [ ] 外部共有一覧レポート（ISMS A.5.15対応）
      - [ ] 現在のリスク状況レポート（ISMS 9.1対応）
    - [ ] 期間選択フィルター（過去7日/30日/90日/365日）
    - [ ] スキャン選択ドロップダウン（スキャン必要レポート用）
    - [ ] 「レポート生成」ボタンでレポート表示
    - [ ] サマリー情報が表示される
    - [ ] JSONダウンロードボタン
    - [ ] CSVダウンロードボタン
    ...
    - [ ] メンバー権限のユーザーはアクセス不可（警告表示）
  - [ ] **一括操作機能** ⏳ 未テスト
    - [ ] 「一括操作」ボタンが表示される（管理者のみ）
    - [ ] ボタンクリックで選択モードに切り替わる
    - [ ] 各ファイル行にチェックボックスが表示される
    - [ ] チェックボックスでファイルを選択できる
    - [ ] 選択件数が「X件選択中」と表示される
    - [ ] 選択中のファイルがハイライト表示される
    - [ ] 一括操作バー表示
      ...
      - [ ] 選択解除ボタン（×アイコン）
    - [ ] 操作ボタンクリックで確認ダイアログ表示
    - [ ] 確認ダイアログに影響ファイル数が表示される
    - [ ] 「実行する」で操作が実行される
    - [ ] 実行結果がトースト通知で表示される（成功/失敗件数）
    - [ ] 操作後、ファイル一覧が更新される
    - [ ] 外部オーナーファイルは選択不可
    - [ ] メンバー権限のユーザーは一括操作ボタンが表示されない
  - [ ] **通知設定ページ** (`/notifications`) ⏳ 未テスト
    - [ ] ページタイトル「通知設定」が表示される
    - [ ] タブ切り替え（設定/履歴）が動作する
    - [ ] **設定タブ**
      - [ ] メール通知ON/OFFトグル
      - [ ] 通知受信者一覧が表示される
      - [ ] 受信者追加フォーム（メール入力＋追加ボタン）
      - [ ] 受信者削除ボタン
      - [ ] 通知トリガー設定
        - [ ] スキャン完了時
        - [ ] Criticalリスク検出時
    ...
    - [ ] **履歴タブ**
      - [ ] 通知履歴一覧が表示される
      - [ ] 日時、タイプ、件名、受信者、成功/失敗が表示される
      - [ ] タイプ別バッジ（スキャン完了、Criticalリスク等）
    - [ ] メンバー権限のユーザーは設定変更不可（閲覧のみ）
  - [ ] **権限変更機能（単一ファイル）** ⏳ 未テスト
    - [ ] ファイル詳細モーダルに権限変更ボタンが表示される
    - [ ] 「削除」ボタンクリックで確認ダイアログ表示
    - [ ] 「閲覧者に変更」ボタンクリックで確認ダイアログ表示
    - [ ] 確認後、権限が変更される
    - [ ] 変更結果がトースト通知で表示される
    - [ ] 権限一覧が更新される
    - [ ] ownerの権限は変更ボタンが表示されない
    - [ ] 外部オーナーファイルは変更不可の警告表示
    - [ ] メンバー権限のユーザーは権限変更ボタンが表示されない
  - [ ] **リスク検出精度** ⏳ 未テスト
...
### Domain-Wide Delegation統合スキャン ✅ 完了
...
- [x] サービスアカウント設定
  - [x] GCPでサービスアカウント作成手順ドキュメント
  - [x] 必要なスコープの定義
  - [x] JSONキーのアップロードUI
- [x] Google Workspace管理コンソール設定ガイド
  - [x] Domain-Wide Delegation有効化手順
  - [x] スコープ承認手順
  - [x] 設定確認テスト機能（verify API）
- [x] 統合スキャン機能
  - [x] 全ユーザーリストの取得（Admin SDK）
  - [x] ユーザーごとのimpersonationでスキャン
  - [x] 進捗表示（X/Y ユーザー完了）
  - [x] エラーハンドリング（アクセス拒否ユーザーのスキップ）
- [x] 結果の統合表示
  - [x] 全ユーザーのファイルを統合表示
  - [x] ユーザー別フィルター
  - [x] 組織全体のリスクサマリー
...
**実装内容**:
- バックエンド: `backend/src/services/serviceAccount.ts` - JWT認証、Delegated Drive/Admin APIクライアント
- バックエンド: `backend/src/routes/delegation.ts` - 設定・検証・スキャン開始/状況取得API
- Firestore: `OrganizationService`にサービスアカウント設定管理メソッド追加
- フロントエンド: `frontend/src/pages/DelegationPage.tsx` - 設定UI、5ステップガイド、スキャン進捗表示
- フロントエンド: `frontend/src/lib/api.ts` - delegationApi、integratedScanApi追加
- ナビゲーション: Layout.tsxに「統合スキャン設定」リンク追加

**バックグラウンドスキャン永続化（2025年11月30日追加）**:
- データモデル: `IntegratedScanJob`, `IntegratedScanUserResult` 型定義追加
- Firestore: `IntegratedScanJobService` - ジョブのCRUD・進捗管理
- バックエンド: スキャン状態をFirestoreに永続化（サーバー再起動時も継続）
- バックエンド: スキャンのキャンセル機能（`POST /api/delegation/scan/cancel`）
- バックエンド: 自動再開機能（runningジョブを検出して再開）
- フロントエンド: キャンセルボタン追加、キャンセル済みステータス表示
